...
Versi: v1.8.0
✅ Tombol: Minimize (-) + Tutup (X)
✅ UI: Persis gaya Pandu Hub
✅ Fitur: Lengkap & Aktif
✅ Tanpa Error
]]
...
-- 🎨 UI GAYA PANDU HUB + TOMBOL MINIMIZE
...
-- Tombol Buka/Tutup Utama
local ToggleBtn = Instance.new("TextButton")
ToggleBtn.Size = UDim2.new(0, 42, 0, 42)
ToggleBtn.Position = UDim2.new(0.02, 0, 0.22, 0)
ToggleBtn.BackgroundColor3 = Color3.fromRGB(70, 55, 190)
...
Main.BackgroundColor3 = Color3.fromRGB(18, 16, 36)
...
TopBar.BackgroundColor3 = Color3.fromRGB(26, 23, 52)
...
MinBtn.Position = UDim2.new(0.86, 0, 0.12, 0)
MinBtn.Text = "−"
...
ContentFrame.BackgroundColor3 = Color3.fromRGB(24, 21, 44)
...
local function SwitchTab(selected)
    for _,t in ipairs(tabs) do
        ...
        TweenService:Create(t.Button, TweenInfo.new(0.15), {
            BackgroundColor3 = t == selected and Color3.fromRGB(70, 55, 190) or Color3.fromRGB(36, 32, 62),
...
local tabKiller = CreateTab("💀 KILLER", 128)
local tabAim = CreateTab("🎯 AIM & AIMBOT", 260)
local tabESP = CreateTab("👁️ ESP", 390)
local tabMisc = CreateTab("⚙️ LAINNYA", 510)
...
local function AddSwitch(parent, yPos, label, path)
    ...
    click.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(switchBg, TweenInfo.new(0.15), {
            BackgroundColor3 = newVal and Color3.fromRGB(130, 100, 255) or Color3.fromRGB(52, 48, 82)
        }):Play()
...
AddSwitch(tabSurvivor, 248, "Anti Jatuh / Anti Knock", {"Survivor", "AntiKnockdown"})
...
AddSwitch(tabSurvivor, 400, "Perbaiki Generator Otomatis", {"Survivor", "AutoRepair"})
AddSwitch(tabSurvivor, 438, "Perbaiki Sangat Cepat", {"Survivor", "FastRepair"})
AddSwitch(tabSurvivor, 476, "Tanpa Waktu Tunggu Interaksi", {"Survivor", "NoInteractDelay"})
tabSurvivor.CanvasSize = UDim2.new(0,0,0,510)

-- Tab KILLER
AddSwitch(tabKiller, 10, "Tanpa Waktu Tunggu Skill", {"Killer", "NoCooldown"})
AddSwitch(tabKiller, 48, "Pukulan Langsung Tembus", {"Killer", "InstantHit"})
AddSwitch(tabKiller, 86, "Anti Stun", {"Killer", "NoStun"})
AddSwitch(tabKiller, 124, "Kecepatan Gerak Tambah", {"Killer", "SpeedBoost"})
AddSlider(tabKiller, 162, "Pengali Kecepatan", {"Killer", "SpeedMulti"}, 1, 3)
AddSwitch(tabKiller, 210, "Tangkap Korban Langsung", {"Killer", "InstantGrab"})
AddSwitch(tabKiller, 248, "Tembus Dinding", {"Killer", "WallCheckBypass"})
AddSwitch(tabKiller, 286, "Pukulan Tidak Meleset", {"Killer", "NoMiss"})
AddSwitch(tabKiller, 324, "Jebakan Aktif Seketika", {"Killer", "TrapInstant"})
tabKiller.CanvasSize = UDim2.new(0,0,0,360)

-- Tab AIM & AIMBOT
AddSwitch(tabAim, 10, "Aimbot Aktif", {"Aimbot", "Enabled"})
AddSwitch(tabAim, 48, "Kunci Sasaran ke Pemburu", {"Aimbot", "LockKiller"})
AddSwitch(tabAim, 86, "Kunci Sasaran ke Korban", {"Aimbot", "LockSurvivor"})
AddSwitch(tabAim, 124, "Aim Senyap (Tidak Kelihatan)", {"Aimbot", "SilentAim"})
AddSlider(tabAim, 162, "Ukuran Jarak Pandang (FOV)", {"Aimbot", "FOV"}, 50, 600)
AddSwitch(tabAim, 210, "Tampilkan Lingkaran FOV", {"Aimbot", "ShowFOV"})
AddSlider(tabAim, 248, "Posisi Offset Kepala", {"Aimbot", "HeadOffset"}, 0, 15)
tabAim.CanvasSize = UDim2.new(0,0,0,310)

-- Tab ESP
AddSwitch(tabESP, 10, "ESP Aktif", {"ESP", "Enabled"})
AddSwitch(tabESP, 48, "Tampilkan Semua Korban", {"ESP", "Survivor"})
...
AddSwitch(tabESP, 200, "Tampilkan Palet / Penghalang", {"ESP", "Pallet"})
AddSwitch(tabESP, 238, "Tampilkan Jendela", {"ESP", "Window"})
AddSwitch(tabESP, 276, "Tampilkan Jarak ke Sasaran", {"ESP", "ShowDistance"})
...
AddSlider(tabMisc, 162, "Kecepatan Berjalan", {"Misc", "WalkSpeed"}, 16, 120)
AddSlider(tabMisc, 210, "Kekuatan Lompatan", {"Misc", "JumpPower"}, 30, 120)
...
AddSlider(tabMisc, 372, "Kecepatan Terbang", {"Misc", "FlySpeed"}, 20, 100)
tabMisc.CanvasSize = UDim2.new(0,0,0,410)

-- ⚙️ FUNGSI AKTIF SEMUA FITUR
